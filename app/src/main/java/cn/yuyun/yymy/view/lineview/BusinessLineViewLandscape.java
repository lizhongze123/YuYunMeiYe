...
public class BusinessLineViewLandscape extends View {

    /**
     * 控件默认宽高
     */
    private static final float DEF_WIDTH = 650;
    private static final float DEF_HIGHT = 900;
    ...
    public BusinessLineViewLandscape(Context context) {
        this(context, null);
    }

    public BusinessLineViewLandscape(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BusinessLineViewLandscape(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(AttributeSet attrs) {
        // 读取属性值
        TypedArray ta = mContext.obtainStyledAttributes(attrs, R.styleable.BusinessLineViewLandscape);
        lineColor = ta.getColor(R.styleable.BusinessLineViewLandscape_lineColorLandscape, Color.BLACK);
        ta.recycle();
        initDimen();
        initLoadingPaint();
        initInnerXPaint();
    ...
    private void initDimen() {
        mXYTextSize = mContext.getResources().getDimensionPixelSize(R.dimen.x20);
...
//        Rect rect = getTextBounds("¥" + StringFormatUtils.formatDecimal(textBottom), floatTextPaint);
    ...
    private void drawBrokenPaint(Canvas canvas) {
        ...
        if (sum != 0) {
            ...
            path.moveTo(mPaddingLeft + xInterval / 2, floatY);
            for (int i = 0; i < mFundModeList.size(); i++) {
    ...
    private void drawYPaint(Canvas canvas) {
        ...
        float textHight = metrics.descent - metrics.ascent;
        //现将最小值、最大值画好
        //draw min
        float minWigth = mXYPaint.measureText((int) mMinFundMode.dataY + "") + mLeftTxtPadding;
        canvas.drawText((int) mMinFundMode.dataY + "",
                mPaddingLeft - minWigth,
                mHeight - mPaddingBottom, mXYPaint);
        //draw max
        float maxWigth = mXYPaint.measureText((int) mMaxFundMode.dataY + "") + mLeftTxtPadding;
        canvas.drawText((int) mMaxFundMode.dataY + "",
                mPaddingLeft - maxWigth,
                mPaddingTop + textHight / 3, mXYPaint);
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            float width = mXYPaint.measureText(text) + mLeftTxtPadding;
            canvas.drawText(text,
                    mPaddingLeft - width,
                    mHeight - mPaddingBottom - perYWidth * i + textHight / 3, mXYPaint);
        }
    }
    ...
    private void drawXPaint(Canvas canvas) {
        ...
        xInterval = (mWidth - mPaddingLeft - mPaddingRight) / (mFundModeList.size());
        if (type == TYPE_WEEK) {
            for (int i = 0; i < mFundModeList.size(); i++) {
                String text = mFundModeList.get(i).datetime.substring(5, mFundModeList.get(i).datetime.length()) + "(" + mFundModeList.get(i).week + ")";
                float textWidth = mXYPaint.measureText(text);
                canvas.drawText(text, mPaddingLeft + xInterval * i + textWidth / 2, hight, mXYPaint);
            }
        } else {
            for (int i = 0; i < mFundModeList.size(); i++) {
                String text = mFundModeList.get(i).datetime.substring(8, mFundModeList.get(i).datetime.length());
                float textWidth = mXYPaint.measureText(text);
                canvas.drawText(text, mPaddingLeft + xInterval * i + xInterval / 2 - textWidth / 2, hight, mXYPaint);
            }
        }
    }
    ...
    private void drawInnerXPaint(Canvas canvas) {
        ...
        canvas.drawLine(0 + mPaddingLeft, mPaddingTop + perYWidth * 0,
                mWidth - mPaddingRight, mPaddingTop + perYWidth * 0, linePaint);

        canvas.drawLine(0 + mPaddingLeft, mPaddingTop + perYWidth * 1,
                0 + mWidth - mPaddingRight, mPaddingTop + perYWidth * 1, linePaint);

        canvas.drawLine(0 + mPaddingLeft, mPaddingTop + perYWidth * 2,
                0 + mWidth - mPaddingRight, mPaddingTop + perYWidth * 2, linePaint);

        canvas.drawLine(0 + mPaddingLeft, mPaddingTop + perYWidth * 3,
                0 + mWidth - mPaddingRight, mPaddingTop + perYWidth * 3, linePaint);

        canvas.drawLine(0 + mPaddingLeft, mPaddingTop + perYWidth * 4,
                0 + mWidth - mPaddingRight, mPaddingTop + perYWidth * 4, linePaint);
    }
