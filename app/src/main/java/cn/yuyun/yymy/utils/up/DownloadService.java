...
import cn.lzz.utils.StreamUtil;
...
public class DownloadService extends Service {
    ...
    // 默认存放文件下载的路径
    public  String saveFileName= Environment.getExternalStorageDirectory()
            + File.separator
            + "download"
            + File.separator + "YYMY" + File.separator;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        final File apkFile = new File(saveFileName + "yymy.apk");
        setUpNotification();
        new Thread() {
            @Override
            public void run() {
...
//        return super.onStartCommand(intent, flags, startId);
    ...
    private void installApk() {
        LogUtils.e("开始安装");
        File file = new File(saveFileName + "yymy.apk");
        if (!file.exists()){
            return;
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            Uri contentUri = FileProvider.getUriForFile(getApplicationContext(), "cn.yuyun.yymy.fileprovider", file);
