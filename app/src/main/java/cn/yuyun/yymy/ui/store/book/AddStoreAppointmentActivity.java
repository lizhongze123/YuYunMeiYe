...
import cn.yuyun.yymy.ui.home.appointment.ProjectListActivity;
...
import cn.yuyun.yymy.ui.store.staff.SelectStaffListActivity;
 ...
 * @date
 */
public class AddStoreAppointmentActivity extends BaseActivity {

    private final int REQUEST_PROJECT = 1115;
    private final int REQUEST_STAFF = 1117;
    ...
    public static Intent startAddStoreAppointmentActivity(Context context, StoreBean bean) {
        return new Intent(context, AddStoreAppointmentActivity.class).putExtra(EXTRA_DATA, bean);
    }

    public static Intent startAddStoreAppointmentActivity(Context context, StoreBean bean, Bundle bundle) {
        return new Intent(context, AddStoreAppointmentActivity.class).putExtra(EXTRA_DATA, bean).putExtra(EXTRA_DATA2, bundle);
    }
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_store_appointment);
    }
    ...
    private void initViews() {
        ...
        Bundle bundle = getIntent().getBundleExtra(EXTRA_DATA2);
        ...
        if(null != bundle){
            ...
            tvDate.setText(date);
            TextViewUtils.setTextOrEmpty(tvStaff, bundle.getString("staffName"));
            List<String> mechanicList = new ArrayList<>();
            mechanicList.add(bundle.getString("staffId"));
            requestBean.mechanic_list = mechanicList;
        }

        titleBar.setTilte("添加门店预约");
        titleBar.setTvRight("提交");
        titleBar.setRightOnClicker(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (dataCheck()) {
                    requestBean.startTime = DateTimeUtils.parseToStamp(date + " " + startTime, DateTimeUtils.FORMAT_DATETIME);
                    requestBean.endTime = DateTimeUtils.parseToStamp(date + " " + endTime, DateTimeUtils.FORMAT_DATETIME);
        ...
        llProject.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivityForResult(ProjectListActivity.startProjectListActivity(mContext, storeBean, mIntentResult), REQUEST_PROJECT);
            }
        });
        llStaff.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = SelectStaffListActivity.startSelectStaffListActivity(mContext, storeBean, intentData);
        ...
        for (int i = 0; i < 30; i++) {
            dateItems.add(DateTimeUtils.getCustomDay2(i, DateTimeUtils.FORMAT_DATE_UI));
        }
        datePickerView = new OptionsPickerBuilder(mContext, new OnOptionsSelectListener() {
            ...
            public void onOptionsSelect(int options1, int options2, int options3, View v) {
                ...
                date = DateTimeUtils.getCustomDay2(options1, DateTimeUtils.FORMAT_DATE_UI);
            }
        }).build();
    ...
    private boolean dataCheck() {
        ...
        if (TextUtils.isEmpty(tvName.getText().toString())) {
            ...
            tips = "请选择门店";
        } else if (TextUtils.isEmpty(tvProject.getText().toString())) {
            tips = "请选择项目";
    ...
    private List<StaffBean> intentData;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (data != null && resultCode == RESULT_OK) {
            ...
            } else if (REQUEST_STAFF == requestCode) {
                intentData = (List<StaffBean>) data.getSerializableExtra(EXTRA_DATA);
                StringBuilder sb = new StringBuilder();
                List<String> mechanicList = new ArrayList<>();

                for (int i = 0, len = intentData.size(); i < len; i++) {
                    sb.append(intentData.get(i).staffName + ",");
                    mechanicList.add(intentData.get(i).staffId);
                    if (i == len - 1) {
                        sb.deleteCharAt(sb.toString().length() - 1);
                    }
                }
                TextViewUtils.setTextOrEmpty(tvStaff, sb.toString());
                requestBean.mechanic_list = mechanicList;
            }
        }
    }
