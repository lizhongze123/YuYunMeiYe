 ...
 * @date
 */
public class AssetServiceAdapter extends RecyclerView.Adapter<AssetServiceAdapter.ViewHolder> {

    private int RESOURCE_ID = R.layout.item_asset;
    private Context mContext;
    private List<ResultServiceAsset> ingList = new ArrayList<>();
    private List<ResultServiceAsset> finishList = new ArrayList<>();
    private List<ResultServiceAsset> dataList = new ArrayList<>();
    ...
    public AssetServiceAdapter(Context context, boolean isCashier){
        this.mContext = context;
    ...
    public void notifyDataSetChanged(List<ResultServiceAsset> dataList, int type) {
        this.dataList.clear();
        if(type == 0){
    ...
    public void addAll(List<ResultServiceAsset> dataList) {
        for (ResultServiceAsset bean : dataList) {
            if(bean.status == 1 && bean.expired == Expired.ING){
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private void bindItem(final ResultServiceAsset bean, final int position) {
            if(isCashier){
            ...
            DateTimeUtils.parseToStamp(bean.dead_line, DateTimeUtils.FORMAT_DATETIME_UI);
            ...
            TextViewUtils.setTextOrEmpty(tvName, bean.service_name);
    ...
    public interface OnMyItemClickListener {
        void onItemClick(View view, ResultServiceAsset bean, int position);
    }
