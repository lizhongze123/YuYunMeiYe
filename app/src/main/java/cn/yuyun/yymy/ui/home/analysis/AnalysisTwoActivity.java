...
public class AnalysisTwoActivity extends BaseActivity {
    ...
    private AnalysisTwoRightAdapter rightAdapter;
    ...
    public static Intent startAnalysisTwoActivity(Context context, String id, StoreBean storeBean, RequestAnalysisOne body) {
        return new Intent(context, AnalysisTwoActivity.class).putExtra(EXTRA_DATA, id).putExtra(EXTRA_DATA2, storeBean).putExtra(EXTRA_TYPE, body);
    }

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_analysis_two);
    }
    ...
    private void initData() {
        ...
        if(id.equals("无品牌(项目)") || id.equals("无品牌(产品)") || id.equals("无品牌(套餐)")){
            requestReport.good_brand_id = "";
            requestReport.good_brand_name = id;
        }else{
            requestReport.good_brand_id = id;
    ...
    private void initViews() {
        ...
        rightAdapter = new AnalysisTwoRightAdapter(mContext);
        ...
        rightAdapter.setOnItemClickListener(new AnalysisTwoRightAdapter.OnMyItemClickListener() {
            @Override
            public void onItemClick(ResultAnalysisOne bean, int position) {
                startActivity(AnalysisThreeActivity.startAnalysisThreeActivity(mContext, bean, storeBean, requestReport));
            }
        });
    }
    ...
    private void configurePresenter() {
        ...
        presenter.setLoadDataListener(new Presenter2.OnPresenterLoadListener<ResultAnalysisOne>() {
            ...
            public void onLoad(Subscriber<DataBean<PageInfo<ResultAnalysisOne>>> subscriber, int pageIndex, int pageSize) {
                ...
                AppModelFactory.model().getAnalysisTwo(requestReport, subscriber);
            }
        });
    }

    private void getTotal() {
        AppModelFactory.model().getAnalysisTwoTotal(requestReport, new NoneProgressSubscriber<DataBean<ResultAnalysisOneTotal>>(mContext) {
